...
export default function InsuranceRequestPage() {
  ...
  const buildDetails = () => {
    const lines = [
      `מיקום טיפול: ${formData.locations.join(", ") || "לא צוין"}`,
      `מטופלים בשבוע: ${formData.patientsPerWeek || "לא צוין"}`,
      `ביטוח קיים: ${
        formData.hasCurrentInsurance === "yes"
          ? `כן (${formData.currentInsurer || "לא צוין"})`
          : formData.hasCurrentInsurance === "no"
            ? "לא"
            : "לא צוין"
      }`,
      `סכום כיסוי מבוקש: ${formData.coverageAmount || "לא צוין"}`,
      `כיסוי צד ג': ${formData.thirdPartyCoverage ? "כן" : "לא"}`,
      `כיסוי אונליין: ${formData.onlineCoverage ? "כן" : "לא"}`,
    ]
